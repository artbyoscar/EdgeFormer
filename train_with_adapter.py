...
def main():
    print("Training EdgeFormer with adapter...")
    
    # Load processed dataset
    dataset_path = "data/focused/processed_dataset.pt"
    if not os.path.exists(dataset_path):
        print(f"Dataset not found at {dataset_path}")
        print("Running process_dataset.py first...")
        
        # Load dataset
        raw_dataset_path = "data/focused/text_dataset.pt"
        dataset = torch.load(raw_dataset_path)
        print(f"Loaded raw dataset with length: {len(dataset)}")
        
        # Create sequences
        seq_length = 48
        sequences = []
        
        # Create overlapping sequences (if dataset is a list of tokens)
        if isinstance(dataset, list):
            ...
            torch.save(sequences_tensor, dataset_path)
    ...
    # Create a simple model (instead of EdgeFormer)
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, 'checkpoints/simple_best_model_new.pt')
        ...
        if (epoch + 1) % 10 == 0:
            torch.save({
                ...
                'num_layers': num_layers,
                'vocab_size': vocab_size
            }, f'checkpoints/simple_model_epoch_{epoch+1}_new.pt')
    ...
    torch.save({
        ...
        'num_layers': num_layers,
        'vocab_size': vocab_size
    }, 'checkpoints/simple_final_model_new.pt')
