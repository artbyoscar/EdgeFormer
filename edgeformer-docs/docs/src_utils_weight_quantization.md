...
```python
__init__(self, orig_layer, weight, scales, zeros, bits=4, group_size=128, symmetric=True)
```
...
```python
compare_quantization_configs(model, inputs, configs=None, output_dir=None)
```
...
```python
kv_cache_offload(model, offload_path=None, kv_cache_dtype=torch.float16)
```
...
```python
quantize_weight_only(module, bits=4, group_size=128, symmetric=True)
```
...
```python
test_weight_only_quantization(model, inputs, bits=4, group_size=128, symmetric=True)
```

### weight_only_quantize_model
...
```python
weight_only_quantize_model(model, bits=4, group_size=128, symmetric=True, modules_to_exclude=None)
```
