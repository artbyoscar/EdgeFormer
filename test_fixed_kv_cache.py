...
def test_kv_cache_manager():
    ...
    cache_manager = KVCacheManager(
        ...
        num_heads=num_heads,
        head_dim=head_dim,
        max_gpu_cache_size=64,  # 64 MB max GPU cache size
        enable_offload=True,
    ...
    for seq_len in sequence_lengths:
        ...
        tokens_per_update = 16  # Smaller chunks
        updates_needed = seq_len // tokens_per_update
        
        for i in range(updates_needed):
            # Create dummy key and value tensors - with proper shape
            key = torch.rand(
                max_batch_size,  # batch size
                tokens_per_update,  # sequence length
                num_heads,  # num heads
